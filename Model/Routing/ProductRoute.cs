...
{
    public class ProductRoute : Route, IRouteWithArea
    {
        ...
        public ProductRoute(string area)
        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            var urlmatch = Regex.Match(url, @"^(\w{2})/(\d*)-([^./]*)/((\d+)-([^./]+)/)*(\d+)-([^./]+).html", RegexOptions.IgnoreCase);
            if (urlmatch.Success)
            {
                ...
                routeData.Values.Add("id", urlmatch.Groups[urlmatch.Groups.Count-2].Value);
                routeData.Values.Add("title", urlmatch.Groups[urlmatch.Groups.Count - 1].Value);

                List<string> categories = new List<string>();
                for (int i = 2; i < urlmatch.Groups.Count-3; i+=(i >= 4 ? 3 : 2) )
                {
                    categories.Add(String.Format("{0}-{1}", urlmatch.Groups[i].Value, urlmatch.Groups[i+1].Value));
                }

                routeData.Values.Add("Categories", categories.Where(x=>x != "-").ToArray());
        ...
        public override VirtualPathData GetVirtualPath(RequestContext requestContext, RouteValueDictionary values)
        {
            ...
            if (values.ContainsKey("action") && (!string.Equals(Action, values["action"] as string, StringComparison.InvariantCultureIgnoreCase)))
                return null;
            if ((!values.ContainsKey("Categories[0]")) || (!values.ContainsKey("Culture")) || (!values.ContainsKey("Id")) || (!values.ContainsKey("Title")))
                return null;

            StringBuilder categoryPath = new StringBuilder();
            int i = 0;
            while (values.ContainsKey("Categories["+i+"]"))
            {
                categoryPath.AppendFormat("{0}/", values["Categories[" + i + "]"]);
                ++i;
            }

            return new VirtualPathData(this, string.Format("{0}/{1}/{2}-{3}.html", values["Culture"], categoryPath, values["Id"], values["Title"]));
        }
    }
}
