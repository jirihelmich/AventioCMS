...
{
    public class PageRoute : Route, IRouteWithArea
    {
        public string Controller { get; set; }
        public string Action { get; set; }
        public string Area { get; private set; }

        //{culture}/{Id}-{Title}.html

        public PageRoute(string area) : base(String.Empty, new MvcRouteHandler())
        {
            Area = area;
        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            var urlmatch = Regex.Match(url, @"^(\w{2})/(\d*)-([^./]*).html", RegexOptions.IgnoreCase);
            if (urlmatch.Success)
            {
                ...
                routeData.Values.Add("id", urlmatch.Groups[2].Value);
                routeData.Values.Add("title", urlmatch.Groups[3].Value);
