...
{
    public class CategoryRoute : Route, IRouteWithArea
    {
        ...
        public CategoryRoute(string area)
        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            var urlmatch = Regex.Match(url, @"(\w{2})/(\d*)-([^./]*)/((\d*)-([^./]*))*", RegexOptions.IgnoreCase);
            if (urlmatch.Success)
            {
                ...
                for (int i = 2; i < urlmatch.Groups.Count; i+=(i >= 4 ? 3 : 2) )
                {
        ...
        public override VirtualPathData GetVirtualPath(RequestContext requestContext, RouteValueDictionary values)
        {
            ...
            if (values.ContainsKey("action") && (!string.Equals(Action, values["action"] as string, StringComparison.InvariantCultureIgnoreCase)))
                return null;
            if ((!values.ContainsKey("Categories[0]")) || (!values.ContainsKey("culture")))
                return null;
            ...
            return new VirtualPathData(this, string.Format("{0}/{1}", values["culture"], categoryPath));
        }
    }
}
